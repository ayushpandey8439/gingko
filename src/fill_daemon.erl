...
build(TxId, Key, Type, BaseSnapshot, MinSnapshotTime, MaximumSnapshotTime, Partition) ->
  ...
  {ok, Data} = gingko_op_log:read_log_entries(Key, ContinuationObject, Partition),
  ...
  % The payload for the key can also comtain entries that are not possibly committed. These can be added back to the payload entry and that can be used to read items witnin the
